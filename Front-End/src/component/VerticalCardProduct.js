      ...
      <div className="flex items-center gap-4 md:gap-6 overflow-scroll scrollbar-none">
        {loading
          ? loadingList.map((_, ind) => {
              return (
                <div
                  className="w-full min-w-[180px] md:min-w-[220px] max-w-[180px] md:max-w-[220px] bg-white rounded-sm shadow"
                  key={ind + 1}
                >
          ...
          : data.map((product, index) => {
              return (
                <NavLink
                  ...
                  key={index + 1}
                >
                  <div className="bg-slate-100 h-48 p-4 min-w-[145px] flex justify-center items-center">
                    <img
                      src={product?.ProductImage[0]}
                      className="object-scale-down h-full w-full hover:scale-110 cursor-pointer transition-all bg-slate mix-blend-multiply"
                    />
                  </div>
                  <div className="p-4 grid gap-2">
